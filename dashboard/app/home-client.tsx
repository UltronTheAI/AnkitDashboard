...
const LOCK_UNTIL_KEY = "ankit_daily_lock_until";
...
function loadLockUntil(): number | null {
  try {
...
function saveLockUntil(msEpoch: number | null) {
  try {
    if (msEpoch === null) localStorage.removeItem(LOCK_UNTIL_KEY);
    else localStorage.setItem(LOCK_UNTIL_KEY, String(msEpoch));
  } catch {}
}
...
export default function HomeClient() {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(t);
  }, [isLocked, lockUntil, msUntilNext, nowMs]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!canSubmit) {
      setStatus({
        type: "err",
        message: `Daily limit reached (${REQUEST_LIMIT}). Try again in ${
          cooldownText ?? "a bit"
        }.`,
      });
      return;
    ...
    try {
      ...
      const now = Date.now();
      const next = [...loadRecentRequestTimestamps(), now];
      ...
      if (filtered.length >= REQUEST_LIMIT) {
        ...
        const until = oldest + REQUEST_WINDOW_MS;
        setNowMs(now);
        setLockUntil(until);
        saveLockUntil(until);
      }
    ...
    <div className="space-y-10">
      <section className="rounded-2xl border border-black/5 bg-white/70 p-6 shadow-sm dark:border-white/10 dark:bg-black/40">
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          This site is for requesting resources from Ankit. Fill the form, pick
          exactly one resource, and you’ll receive it by email. Daily limit:{" "}
          {REQUEST_LIMIT} requests per 24 hours.
          {savedInfo ? (
            <>
              {" "}
              (Autofilled from{" "}
